...
seqdata <- seqdef(dat[, time_cols, drop = FALSE], event_labels_order = states, id = dat[[id_col]])
print(dim(seqdata))
...
Subsequences are given as strings: each transition (or event) in parentheses, separated by `-`, e.g. `"(1-2)-(2-3)"` (format depends on `alphabet(eseq)`). Events must be from the event_labels_order of `eseq`. See `str.seqelist` in TraMineR for the display format.

```{r}
#| label: str-subseq
#| eval: true
# Example: search for specific patterns (use labels from event_labels_order(eseq))
alph <- event_labels_order(eseq)
str_subseq <- c(
...
|------|--------------|-------------------|
| Load & state sequence | `read.csv` + `seqdef(..., event_labels_order, id)` | `load_dataset` + `SequenceData(..., time, id_col, states)` |
| Event sequences | `seqecreate(seqdata, tevent = "transition")` | `EventSequenceData.from_state_sequences(seqdata=seqdata, event_representation="transition")` |
| Constraints | `seqeconstraint(max.gap, window.size, age.min, age.max, count.method)` | `EventSequenceConstraint(...)` |
| Frequent subsequences | `seqefsub(eseq, min.support, constraint, str.subseq)` | `find_frequent_subsequences(event_sequences=eseq, min_support=min_support, search_constraint=constraint, target_subsequences=str_subseq)` |
| Occurrence matrix | `seqeapplysub(fsub, method = "presence" \| "count")` | `count_subsequence_occurrences(subsequence_results=fsub, counting_method="presence" \| "count")` |
| Group comparison | `seqecmpgroup(fsub, group, method="chisq", pvalue.limit)` | `compare_groups(subsequence_results=fsub, group_labels=group, test_method="chisq", pvalue_threshold=0.05)` |
| Plot event sequences | `plot(eseq)` / `seqpcplot(eseq)`; `plot(eseq, group=...)` | `plot_event_parallel_coordinates(event_sequences=eseq, group_labels=group)` |
| Plot subseq frequencies | `plot(fsub[1:n])`; `plot(..., freq=Count)` | `plot_subsequence_frequencies(subsequence_results=fsub, frequency_values=fsub.data["Count"])` |
