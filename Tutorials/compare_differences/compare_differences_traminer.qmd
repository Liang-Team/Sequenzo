...
| `get_group_differences()` | `seqCompare()` |
| `get_lrt_test()` | `seqLRT()` |
| `get_bic_test()` | `seqBIC()` |
...
```{r}
#| label: group-differences-methods
#| eval: true
# LCS method
set.seed(36963)
...
```{r}
#| label: group-differences-sampling
#| eval: true
...
```{r}
#| label: group-differences-weighted
#| eval: true
# Weighted (default)
set.seed(36963)
...
| Overall comparison | `seqCompare(seqdata, group, method, s, stat)` | `get_group_differences(seqdata, group, method, s, stat)` |
| LRT only | `seqLRT(seqdata, group, method, s)` | `get_lrt_test(seqdata, group, method, s)` |
| BIC only | `seqBIC(seqdata, group, method, s)` | `get_bic_test(seqdata, group, method, s)` |
