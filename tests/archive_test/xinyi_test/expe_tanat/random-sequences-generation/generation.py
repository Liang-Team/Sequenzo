...
helpstring="""generation.py -n <val> -l <val> --np=<val> --lp=<val> --th=<val> -d <val> -o <outputfile> --asp
    ...
    * U: uniqueness rate 0-100, for type 'random' only. U=5 means 5%% of sequences are unique, 95%% are duplicates (default: 100)
...
Usage example:
    ...
    $ python generation.py -l 10 -d 20 -c 0.6 --ibm -o output.dat
    ...
    $ python generation.py -l 10 -d 20 -c 0.6 --discr -g 3 --ibm -o output.dat
    ...
    $ python generation.py -l 10 -d 20 --neg --ibm -o output.dat
    This command generates two files:
...
def main(argv):
    ...
    tanat=False
    fixedlength = False
    uniqueness_rate = 100  # for random type: U% unique sequences
    patterntype = "sequential" # ['sequential', 'temporal', 'negative', 'chronicle', 'random']
    
    ## Read parameters
    try:
       opts, args = getopt.getopt(argv,"hn:d:l:c:o:t:i",["pd","ibm","asp","fl","Mlambda=","mlambda=","nbseq=","nbitems=","length=","type=","ofile=", "np=", "lp=", "th=", "mg=", "U="])
    ...
    for opt, arg in opts:
       if opt == '-h':
           ...
           try:
               uniqueness_rate = float(arg)
           except ValueError:
               warnings.warn("error with argument --U: a number must be given")
               return
           if uniqueness_rate < 0 or uniqueness_rate > 100:
               warnings.warn("error with argument --U: invalid value, must be in [0,100].")
               return

    # 当 -o 为简单文件名时，自动输出到 generated_datasets/<stem>/ 目录
    if "/" not in outputfile and "\\" not in outputfile and not os.path.isabs(outputfile):
        script_dir = os.path.dirname(os.path.abspath(__file__))
        expe_tanat_dir = os.path.dirname(script_dir)
        generated_dir = os.path.join(expe_tanat_dir, "generated_datasets")
        stem = os.path.splitext(os.path.basename(outputfile))[0]
        output_subdir = os.path.join(generated_dir, stem)
        os.makedirs(output_subdir, exist_ok=True)
        outputfile = os.path.join(output_subdir, os.path.basename(outputfile))
    ...
    if patterntype=='sequential':
    ...
    elif patterntype=='temporal_small':
        generator = chrogen.chrodb_generator(nbitems=maxitems, l = length, lp=lengthpatterns, fl="uniform", dc= cd, minstart=0, maxstart=5, minduration=1, maxduration=2, seq_duration=100)
        ...
        sequences = gen_seqdb(seqnb, length, maxitems, fixedlength, uniqueness_rate=uniqueness_rate)
        
        ## Output sequences
        fout = open(outputfile, "w")
        if tanat:
            fout.write("id,stime,etime,event\n")
        for s in sequences:
            if ibm:
                fout.write(s.write_IBM())
            elif tanat:
                fout.write(s.write_tanatcsv())
            else:
                fout.write(str(s)+"\n")
        fout.close()
