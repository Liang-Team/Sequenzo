...
write.csv(
  data.frame(key = "loglik", value = loglik),
  file.path(outdir, "ref_mnhmm_loglik.csv"),
  row.names = FALSE
)

post <- posterior_cluster_probabilities(model)
write.csv(
  as.data.frame(post),
  file.path(outdir, "ref_mnhmm_cluster.csv"),
  row.names = FALSE
)

cat("Written MNHMM fixed references to", outdir, "\n")
