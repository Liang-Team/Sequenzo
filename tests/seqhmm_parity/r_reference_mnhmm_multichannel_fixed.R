...
emission_probs <- list(
  list(
    matrix(c(0.98, 0.02, 0.70, 0.30), nrow = 2, byrow = TRUE),
    matrix(c(0.97, 0.03, 0.65, 0.35), nrow = 2, byrow = TRUE)
  ),
  list(
    matrix(c(0.30, 0.70, 0.02, 0.98), nrow = 2, byrow = TRUE),
    matrix(c(0.35, 0.65, 0.03, 0.97), nrow = 2, byrow = TRUE)
  )
)
...
write.csv(
  ...
  file.path(outdir, "ref_mnhmm_multichannel_loglik.csv"),
  row.names = FALSE
)

write.csv(
  as.data.frame(posterior_cluster_probabilities(model)),
  file.path(outdir, "ref_mnhmm_multichannel_posterior_cluster.csv"),
  row.names = FALSE
)
...
write.csv(
  ...
  file.path(outdir, "ref_mnhmm_multichannel_emission.csv"),
  row.names = FALSE
)

cat("Written multichannel MNHMM fixed references to", outdir, "\n")
