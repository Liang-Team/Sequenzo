...
### 1. `test_discrepancy_analysis_lsog.py` - Functional Testing
...
pytest tests/discrepancy_analysis/ -v

# Run only functional tests (no TraMineR reference files needed)
pytest tests/discrepancy_analysis/test_discrepancy_analysis_lsog.py -v

# Run only consistency tests (requires reference files)
pytest tests/discrepancy_analysis/test_traminer_consistency.py -v

# Run specific test
pytest tests/discrepancy_analysis/test_discrepancy_analysis_lsog.py::test_compute_pseudo_variance_unweighted -v
```
...
|----------|---------------|-----|
| **Daily development** | `test_discrepancy_analysis_lsog.py` | Fast, no setup needed, catches bugs early |
| **CI/CD pipeline** | `test_discrepancy_analysis_lsog.py` | No external dependencies, runs quickly |
...
| **No R/TraMineR installed** | `test_discrepancy_analysis_lsog.py` | Works without external dependencies |
...
4. Save numerical results to CSV files in `tests/discrepancy_analysis/`:
