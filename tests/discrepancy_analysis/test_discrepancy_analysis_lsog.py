...
@File    : test_discrepancy_analysis_lsog.py
...
# ============================================================================
# Test overall_discrepancy (dissvar)
# ============================================================================

def test_overall_discrepancy_unweighted(lsog_distance_matrix):
    """Test pseudo-variance computation without weights."""
    variance = overall_discrepancy(lsog_distance_matrix, weights=None, squared=False)
...
def test_overall_discrepancy_squared(lsog_distance_matrix):
    """Test pseudo-variance computation with squared distances."""
    variance_normal = overall_discrepancy(
        lsog_distance_matrix, weights=None, squared=False
    )
    variance_squared = overall_discrepancy(
...
# ============================================================================
# Test single_factor_association (dissassoc)
# ============================================================================

def test_single_factor_association_basic(lsog_distance_matrix, lsog_predictors):
...
def test_single_factor_association_with_permutation(lsog_distance_matrix, lsog_predictors):
...
# ============================================================================
# Test multifactor_association (multi-factor association wrapper)
# ============================================================================


def test_marginal_factor_association_matches_single_factor_calls(lsog_distance_matrix, lsog_predictors):
    """
    marginal_factor_association should return the same Pseudo F and Pseudo R² as calling
    single_factor_association() separately for each factor.
    """
    factors = lsog_predictors.copy()

    mf_result = marginal_factor_association(
        distance_matrix=lsog_distance_matrix,
        factors=factors,
        weights=None,
        R=0,
    ...
    # For each factor, compare against a direct single_factor_association() call
...
def test_multifactor_association_returns_multifactor_summary(lsog_distance_matrix, lsog_predictors):
    """multifactor_association should return a multifactor summary including a Total row."""
    result = multifactor_association(
        distance_matrix=lsog_distance_matrix,
        factors=lsog_predictors,
        weights=None,
        R=0,
        squared=False,
    )
    assert "Total" in result.index
    assert {"PseudoF", "PseudoR2", "p_value"}.issubset(result.columns)


# ============================================================================
# Test merge_cluster_groups (group merging based on Pseudo R²)
# ============================================================================


def test_merge_cluster_groups_stops_without_strict_improvement(lsog_distance_matrix, lsog_predictors):
    """
    TraMineR only merges when quality loss is strictly smaller than the
    current reference quality. Merging two groups down to one usually does
    not meet that criterion, so no merge is recorded.
    """
    groups = lsog_predictors["group"].values

    result = merge_cluster_groups(
        distance_matrix=lsog_distance_matrix,
        group=groups,
        weights=None,
        target_n_groups=1,
    ...
    assert len(np.unique(result["final_group"])) == 2


# ============================================================================
# Test distance_tree (disstree)
# ============================================================================

def test_distance_tree_basic(lsog_distance_matrix, lsog_predictors):
...
def test_distance_tree_with_weights(lsog_distance_matrix, lsog_predictors):
...
# ============================================================================
# Test sequence_tree (seqtree)
# ============================================================================

def test_sequence_tree_basic(lsog_seqdata, lsog_predictors):
...
def test_sequence_tree_with_precomputed_dist(lsog_seqdata, lsog_predictors, lsog_distance_matrix):
