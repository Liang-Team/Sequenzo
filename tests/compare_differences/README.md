...
This directory contains **three complementary test files** with different purposes:
...
### 3. `test_kob_decomposition.py` - Kitagawa–Oaxaca–Blinder (KOB) Decomposition

**Purpose:** Sanity checks for the generic KOB decomposition implementation.

**Characteristics:**
- **No R / TraMineR dependency**: Uses synthetic data only
- **Fast execution**: Single small test verifying algebraic identities
- **Generic**: Does not depend on sequence data, only on NumPy
...
- Variable-level contributions sum to the corresponding global components:
  - Sum of `by_variable["explained"]` equals `explained`
  - Sum of `by_variable["returns"]` equals `unexplained_returns`
...
### Run only the KOB decomposition tests (no R required):
```bash
pytest tests/compare_differences/test_kob_decomposition.py -v
```
