...
Additional "parameter config" tests (Parts 1b, 1c, 2b, 3b): same algorithms with different
parameter values (norm, indel, tpow, expcost, nu, h, prox, kweights, transindel, otto,
previous, add_column). These also compare against TraMineR; refs are generated by the
same R scripts (extra outputs).
...
@pytest.fixture(scope="module")
def ref_dir_omstran():
    """Generate OMstran ref matrices from the same data as seqdata_subset (guarantees data consistency).
    Always runs R script with fixture's CSV; does not use pre-existing ref files."""
    df, _ = _dyadic_children_subset(NROWS)
    with tempfile.NamedTemporaryFile(suffix=".csv", delete=False) as f:
        df.to_csv(f.name, index=False)
        csv_path = f.name
    try:
        ...
        try:
            ...
            pass
    pytest.skip(
        "R/TraMineR not available; run Rscript traminer_reference.R to generate ref_omstran_*.csv"
    )
...
def test_omstran_constant_matches_traminer(seqdata_subset, ref_dir_omstran):
    """OMstran: transindel=constant, otto=0.5, previous=False, add_column=True, norm=YujianBo vs TraMineR."""
    D_ref = _load_ref_matrix(ref_dir_omstran, "omstran")
    if D_ref is None:
        pytest.skip("ref_omstran.csv not found")
    D = get_distance_matrix(
        seqdata_subset,
        method="OMstran",
        sm="TRATE",
...
def test_omstran_prob_matches_traminer(seqdata_subset, ref_dir_omstran):
    """OMstran: transindel=prob vs TraMineR."""
    D_ref = _load_ref_matrix(ref_dir_omstran, "omstran_prob")
    if D_ref is None:
        pytest.skip("ref_omstran_prob.csv not found")
    D = get_distance_matrix(
        seqdata_subset,
        method="OMstran",
        sm="TRATE",
...
def test_omstran_subcost_matches_traminer(seqdata_subset, ref_dir_omstran):
    """OMstran: transindel=subcost vs TraMineR."""
    D_ref = _load_ref_matrix(ref_dir_omstran, "omstran_subcost")
    if D_ref is None:
        pytest.skip("ref_omstran_subcost.csv not found")
    D = get_distance_matrix(
        seqdata_subset,
        method="OMstran",
        sm="TRATE",
...
def test_omstran_otto03_matches_traminer(seqdata_subset, ref_dir_omstran):
    """OMstran: otto=0.3 vs TraMineR."""
    D_ref = _load_ref_matrix(ref_dir_omstran, "omstran_otto03")
    if D_ref is None:
        pytest.skip("ref_omstran_otto03.csv not found")
    D = get_distance_matrix(
        seqdata_subset,
        method="OMstran",
        sm="TRATE",
...
def test_omstran_otto07_matches_traminer(seqdata_subset, ref_dir_omstran):
    """OMstran: otto=0.7 vs TraMineR."""
    D_ref = _load_ref_matrix(ref_dir_omstran, "omstran_otto07")
    if D_ref is None:
        pytest.skip("ref_omstran_otto07.csv not found")
    D = get_distance_matrix(
        seqdata_subset,
        method="OMstran",
        sm="TRATE",
...
def test_omstran_previous_matches_traminer(seqdata_subset, ref_dir_omstran):
    """OMstran: previous=True vs TraMineR."""
    D_ref = _load_ref_matrix(ref_dir_omstran, "omstran_previous")
    if D_ref is None:
        pytest.skip("ref_omstran_previous.csv not found")
    D = get_distance_matrix(
        seqdata_subset,
        method="OMstran",
        sm="TRATE",
...
def test_omstran_add_column_false_matches_traminer(seqdata_subset, ref_dir_omstran):
    """OMstran: add_column=False vs TraMineR."""
    D_ref = _load_ref_matrix(ref_dir_omstran, "omstran_addcolumn_false")
    if D_ref is None:
        pytest.skip("ref_omstran_addcolumn_false.csv not found")
    D = get_distance_matrix(
        seqdata_subset,
        method="OMstran",
        sm="TRATE",
...
def test_omstran_norm_none_matches_traminer(seqdata_subset, ref_dir_omstran):
    """OMstran: norm=none vs TraMineR."""
    D_ref = _load_ref_matrix(ref_dir_omstran, "omstran_norm_none")
    if D_ref is None:
        pytest.skip("ref_omstran_norm_none.csv not found")
    D = get_distance_matrix(
        seqdata_subset,
        method="OMstran",
        sm="TRATE",
...
def test_omstran_indel1_matches_traminer(seqdata_subset, ref_dir_omstran):
    """OMstran: indel=1 (scalar) vs TraMineR."""
    D_ref = _load_ref_matrix(ref_dir_omstran, "omstran_indel1")
    if D_ref is None:
        pytest.skip("ref_omstran_indel1.csv not found")
    D = get_distance_matrix(
        seqdata_subset,
        method="OMstran",
        sm="TRATE",
