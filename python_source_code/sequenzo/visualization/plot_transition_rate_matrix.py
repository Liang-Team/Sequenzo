...
if __name__ == '__main__':
    ...
    df = pd.read_csv(
        '/Users/lei/Library/CloudStorage/OneDrive-Nexus365/sequenzo/test_data/real_data_my_paper/detailed_sequence_10_work_years_df.csv')

    # Load distance matrix
    distance_matrix_path = '/Users/lei/Library/CloudStorage/OneDrive-Nexus365/sequenzo/test_data/real_data_my_paper/detailed_sequences_OM_trate/seqdist_38902_distance_matrix(numpy).pkl'
    with open(distance_matrix_path, 'rb') as file:
        distance_matrix = pickle.load(file)

    # Validate data
    assert len(df) == distance_matrix.shape[0], "Mismatch between dataset and distance matrix size!"

    # Define states
    unique_states_list = ['data', 'data & intensive math', 'hardware', 'research',
                         'software', 'software & hardware', 'support & test']

    sequence_columns_list = [f'C{i}' for i in range(1, 11)]

    # Initialize sequence data
    sequence_data = SequenceData(data=df, var=sequence_columns_list, states=unique_states_list)

    # Performance measurement
    start_time = time.time()
    mem_before = memory_profiler.memory_usage()[0]

    transition_matrix = compute_transition_matrix(sequence_data)

    print_transition_matrix(sequence_data, transition_matrix)

    # Generate plot
    # plot_transition_matrix(sequence_data,
    #                        state_labels=unique_states_list,
    #                        save_as='transition_matrix_plot',
    #                        dpi=200)
    ...
    print(f"Runtime: {elapsed_time:.2f} seconds, Memory usage: {mem_used:.2f} MiB")