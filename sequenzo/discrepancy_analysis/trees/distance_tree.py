...
from ..stats.overall_discrepancy import overall_discrepancy
...
def distance_tree(
    ...
    **Corresponds to TraMineR function: `disstree()`**
    ...
    >>> 
    >>> # Build tree
    >>> tree = distance_tree(dist_matrix, predictors, R=10, pval=0.1)
    ...
    overall_variance = overall_discrepancy(
    ...
    global_assoc = single_factor_association(
        distance_matrix=distance_matrix,
...
def _build_node(
    ...
    This is an internal function called by distance_tree().
...
def _find_best_split(
    ...
    if pd.api.types.is_numeric_dtype(predictors.iloc[:, predictor_col]):
        ...
        for cutpoint in unique_vals[:-1]:  # Don't need last value
            ...
            left_inertia = weighted_inertia_sum(
                distance_matrix,
                left_indices_original,
                weights=weights,
                squared=False,
            )
            right_inertia = weighted_inertia_sum(
                distance_matrix,
                right_indices_original,
                weights=weights,
                squared=False,
            )
            sc_res = left_inertia + right_inertia
            left_var = left_inertia
            right_var = right_inertia
            
            if sc_res < best_SCres:
                best_SCres = sc_res
                split_info = {
                    'lpop': left_weight,
                    'rpop': right_weight,
