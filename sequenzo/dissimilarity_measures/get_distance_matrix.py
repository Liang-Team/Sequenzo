...
def get_distance_matrix(seqdata, method, refseq=None, norm="none", indel="auto", sm=None, with_missing=False, full_matrix=True,
        ...
        if isinstance(refseq, list) and len(refseq) > 1:
            ...
            for i in range(2):
                if not all(isinstance(x, int) and x >= 0 for x in refseq[i]):
                    raise ValueError("[x] When 'refseq' is a list, it must contain two sets of indexes with positive integer values.")
    ...
    # ===========================
    seqdata_num = seqdata.seqdata   # it's dataframe

    if refseq_type == "sets":
        dseqs_num1 = seqdata_num.iloc[refseq[0], :].drop_duplicates()
        nunique1 = len(dseqs_num1)
        dseqs_num2 = seqdata_num.iloc[refseq[1], :].drop_duplicates()
    ...
    # HAM, DHD
        ...
        max_cost = 0
        for i in range(max(seqs_dlens)):  # seqs_dlens has here only one value
