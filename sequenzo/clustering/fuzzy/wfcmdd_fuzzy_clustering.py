...
@Desc    : 
Distance-based fuzzy clustering (WeightedCluster ``wfcmdd``).
"""
...
def _initialize_membership(memb: Union[np.ndarray, Sequence[int]], n: int) -> np.ndarray:
    if isinstance(memb, np.ndarray) and memb.ndim == 2:
        if memb.shape[0] != n:
            raise ValueError("membership rows must match the distance matrix dimension.")
        return np.asarray(memb, dtype=np.float64).copy()
...
def _compute_functional(
    ...
    if method == "PCMdd":
        total = 0.0
        for cluster_idx in range(k_mov + 1):
...
def wfcmdd(
    ...
    """
    Iterative distance-based fuzzy clustering (WeightedCluster ``wfcmdd``).

    Parameters
    ----------
    diss : np.ndarray
        Square distance matrix.
    memb : np.ndarray or sequence of int
        Initial membership matrix (n x k) or medoid seed indices.
    weights : np.ndarray, optional
        Observation weights.
    method : {"NCdd", "HNCdd", "FCMdd", "PCMdd"}, default "FCMdd"
        Fuzzy clustering variant.
    m : float, default 2.0
        Fuzziness exponent (ignored for HNCdd, which uses hard assignments).
    dnoise : float, optional
        Noise distance for NCdd/HNCdd.
    eta : np.ndarray, optional
        Reference distances for PCMdd.
    alpha : float, default 0.001
        Membership convergence threshold.
    iter_max : int, default 100
        Maximum number of iterations.
    verbose : bool, default False
        Print iteration progress.
    dlambda : float, optional
        Adaptive noise scaling for NCdd.

    Returns
    -------
    WfcmddResult
        Membership matrix, medoid indices, objective value, and distances to centers.
    """
    ...
    n = diss.shape[0]
    weights = np.ones(n, dtype=np.float64) if weights is None else np.asarray(weights, dtype=np.float64).reshape(-1)
    ...
    u = _initialize_membership(memb, n)
    k_mov = u.shape[1]
    if method == "PCMdd" and len(eta) != k_mov:
    ...
    while continue_iter:
        for cluster_idx in range(k_mov):
            other = np.delete(u, cluster_idx, axis=1)
            used_medoids = med[:cluster_idx]
            candidates_mask = np.max(other, axis=1) < 1.0
            if used_medoids.size:
        ...
        if iteration > 2:
            continue_iter = (iteration <= iter_max) and (np.max(np.abs(u - u_prev)) > alpha)
        if continue_iter:
            u_prev = u.copy()
            iteration += 1
            if verbose:
