"""
@Author  : Yuqi Liang 梁彧祺
@File    : fuzzy_sequence_plots.py
@Time    : 09/05/2025 10:11
@Desc    :
Membership-weighted sequence plots (WeightedCluster ``fuzzyseqplot``).

Supports index plots (``type="I"``) and state-distribution plots (``type="d"``).
"""
from __future__ import annotations

import warnings
from typing import Any, Dict, Optional, Sequence, Union

import numpy as np
import pandas as pd

from sequenzo.define_sequence_data import SequenceData
from sequenzo.visualization.plot_sequence_index import plot_sequence_index
from sequenzo.visualization.plot_state_distribution import plot_state_distribution


def _validate_membership(
    membership: np.ndarray,
    require_row_stochastic: bool = True,
) -> np.ndarray:
    if membership.ndim != 2:
        raise ValueError("group must be a membership matrix with one row per sequence.")
    if np.any(~np.isfinite(membership)):
        raise ValueError("membership contains NaN or infinite values.")
    if np.any(membership < 0):
        raise ValueError("membership values must be non-negative.")
    row_sums = membership.sum(axis=1)
    if np.any(row_sums <= 0):
        raise ValueError("each row of membership must have a positive sum.")
    if require_row_stochastic and membership.shape[1] > 1:
        if not np.allclose(row_sums, 1.0, atol=0.05):
            warnings.warn(
                "membership rows do not sum to 1; this is unusual for FANNY / FCMdd "
                "fuzzy clustering but allowed for possibilistic partitions (e.g. PCMdd).",
                stacklevel=3,
            )
    return membership


def _membership_matrix(group: Union[np.ndarray, pd.DataFrame]) -> tuple[np.ndarray, list[str]]:
    if isinstance(group, pd.DataFrame):
        names = [str(col) for col in group.columns]
        membership = group.to_numpy(dtype=np.float64, copy=True)
    else:
        membership = np.asarray(group, dtype=np.float64)
        names = [str(idx + 1) for idx in range(membership.shape[1])]
    return _validate_membership(membership), names


def _expand_fuzzy_plot_inputs(
    seqdata: SequenceData,
    membership: np.ndarray,
    cluster_names: Sequence[str],
    membership_threshold: float,
    memb_exp: float,
    use_original_weights: bool,
    level: Optional[str] = None,
) -> Dict[str, Any]:
    n_obs, n_clusters = membership.shape
    if n_obs != len(seqdata.values):
        raise ValueError("group must have one row per sequence.")

    membership = np.power(membership, memb_exp)
    flat_membership = membership.reshape(-1)
    expanded_index = np.repeat(np.arange(n_obs), n_clusters)
    group_labels = np.tile(np.asarray(cluster_names, dtype=object), n_obs)

    expanded_weights = flat_membership.copy()
    if use_original_weights and seqdata.weights is not None:
        expanded_weights *= np.repeat(seqdata.weights, n_clusters)

    keep = flat_membership >= membership_threshold
    if level is not None:
        keep &= group_labels == level
    if not np.any(keep):
        raise ValueError("No sequence remains after applying the membership threshold.")

    expanded_index = expanded_index[keep]
    group_labels = group_labels[keep]
    expanded_weights = expanded_weights[keep]
    sort_values = flat_membership[keep]

    data = seqdata.data.iloc[expanded_index].reset_index(drop=True)
    expanded_seqdata = SequenceData(
        data=data,
        time=seqdata.time,
        states=seqdata.states,
        labels=seqdata.labels,
        id_col=seqdata.id_col,
        weights=expanded_weights,
        start=seqdata.start,
        custom_colors=seqdata.custom_colors,
    )
    return {
        "seqdata": expanded_seqdata,
        "group_labels": group_labels,
        "sort_values": sort_values,
    }


def _normalize_plot_type(plot_type: str) -> str:
    normalized = plot_type.strip().lower()
    if normalized in {"i", "index"}:
        return "index"
    if normalized in {"d", "distribution"}:
        return "distribution"
    raise ValueError('plot_type must be "index" (or "I") or "distribution" (or "d").')


def fuzzy_sequence_plot(
    seqdata: SequenceData,
    group: Optional[Union[np.ndarray, pd.DataFrame, pd.Series]] = None,
    membership_threshold: float = 0.0,
    plot_type: str = "index",
    use_original_weights: bool = True,
    memb_exp: float = 1.0,
    sort_by: str = "unsorted",
    **kwargs,
):
    """
    Membership-weighted sequence plot (WeightedCluster ``fuzzyseqplot``).

    When ``group`` is ``None`` or a one-dimensional grouping vector, this delegates
    to the standard sequence plotting functions. Otherwise ``group`` must be an
    ``n x k`` membership matrix. Rows are replicated once per cluster and weighted
    by membership strength, matching the augmented-dataset strategy in Studer (2018).

    Parameters
    ----------
    plot_type : {"index", "I", "distribution", "d"}, default "index"
        ``"index"`` / ``"I"``: membership-weighted index plot.
        ``"distribution"`` / ``"d"``: membership-weighted state-distribution plot.
    sort_by : str, default "unsorted"
        Pass ``"membership"`` to sort sequences by membership strength (index plot).
    """
    plot_kind = _normalize_plot_type(plot_type)

    if group is None:
        if plot_kind == "distribution":
            return plot_state_distribution(seqdata, **kwargs)
        return plot_sequence_index(seqdata, **kwargs)

    if isinstance(group, pd.Series) or (isinstance(group, np.ndarray) and group.ndim == 1):
        group_series = pd.Series(group).reset_index(drop=True)
        group_df = pd.DataFrame({seqdata.id_col: seqdata.data[seqdata.id_col], "group": group_series})
        if plot_kind == "distribution":
            return plot_state_distribution(
                seqdata,
                group_dataframe=group_df,
                group_column_name="group",
                **kwargs,
            )
        return plot_sequence_index(
            seqdata,
            group_dataframe=group_df,
            group_column_name="group",
            **kwargs,
        )

    membership, cluster_names = _membership_matrix(group)
    prepared = _expand_fuzzy_plot_inputs(
        seqdata=seqdata,
        membership=membership,
        cluster_names=cluster_names,
        membership_threshold=membership_threshold,
        memb_exp=memb_exp,
        use_original_weights=use_original_weights,
    )

    group_df = pd.DataFrame(
        {
            seqdata.id_col: prepared["seqdata"].data[seqdata.id_col],
            "fuzzy_group": prepared["group_labels"],
        }
    )

    if plot_kind == "distribution":
        return plot_state_distribution(
            prepared["seqdata"],
            group_dataframe=group_df,
            group_column_name="fuzzy_group",
            **kwargs,
        )

    if sort_by == "membership":
        sort_by = "unsorted"
        kwargs["sort_by_ids"] = np.argsort(-prepared["sort_values"])

    return plot_sequence_index(
        prepared["seqdata"],
        group_dataframe=group_df,
        group_column_name="fuzzy_group",
        sort_by=sort_by,
        sort_by_weight=True,
        **kwargs,
    )


def fuzzy_sequence_plot_single(
    seqdata: SequenceData,
    level: Union[int, str],
    group: Union[np.ndarray, pd.DataFrame],
    membership_threshold: float = 0.0,
    plot_type: str = "index",
    use_original_weights: bool = True,
    memb_exp: float = 1.0,
    **kwargs,
):
    """Plot one fuzzy cluster level as a membership-weighted sequence plot."""
    plot_kind = _normalize_plot_type(plot_type)

    membership, cluster_names = _membership_matrix(group)
    prepared = _expand_fuzzy_plot_inputs(
        seqdata=seqdata,
        membership=membership,
        cluster_names=cluster_names,
        membership_threshold=membership_threshold,
        memb_exp=memb_exp,
        use_original_weights=use_original_weights,
        level=str(level),
    )
    sort_by = kwargs.pop("sort_by", "unsorted")
    if plot_kind == "distribution":
        return plot_state_distribution(prepared["seqdata"], sort_by=sort_by, **kwargs)
    if sort_by == "membership":
        kwargs["sort_by_ids"] = np.argsort(-prepared["sort_values"])
    return plot_sequence_index(
        prepared["seqdata"],
        sort_by=sort_by,
        sort_by_weight=True,
        include_legend=False,
        **kwargs,
    )
