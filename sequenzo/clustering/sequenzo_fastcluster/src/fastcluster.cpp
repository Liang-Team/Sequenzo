...
*/
// Conditional FENV_ACCESS pragma - disabled when using fast-math optimization
// This code uses -ffast-math, so FENV_ACCESS is incompatible
// #pragma STDC FENV_ACCESS ON  // Disabled for compatibility
#pragma message("FENV_ACCESS disabled for fast-math compatibility")
#include <fenv.h>
#endif
...
// Portable compatibility macros for compiler-specific features
#ifndef __GNUC__
...
// fc_isnan definition for NaN detection (same as in fastcluster_python.cpp)
...
namespace simd_utils {
    ...
    inline int get_simd_features() {
        int features = SIMD_NONE;
#ifdef __SSE2__
        features |= SIMD_SSE2;
#endif
#ifdef __AVX__
    ...
    inline t_float simd_sqeuclidean(const t_float* a, const t_float* b, t_index dim) {
        t_float sum = 0.0;
        t_index i = 0;
        
#ifdef __AVX2__
        if (get_simd_features() & SIMD_AVX2) {
        ...
        if (get_simd_features() & SIMD_SSE2) {
            ...
            for (; i + 2 <= dim; i += 2) {
                ...
                sum_vec = _mm_add_pd(sum_vec, square);
            }
            // Horizontal sum
            t_float temp[2];
    ...
    inline t_index simd_find_min(const t_float* data, t_index start, t_index end, t_float* min_val) {
        t_float min_value = data[start];
        t_index min_index = start;
        
        t_index i = start + 1;
        
#ifdef __AVX2__
        if (get_simd_features() & SIMD_AVX2 && (end - start) >= 4) {
            __m256d min_vec = _mm256_set1_pd(min_value);
            __m256i indices = _mm256_setr_epi64x(start, start+1, start+2, start+3);
            __m256i step = _mm256_set1_epi64x(4);
            
            for (; i + 4 <= end; i += 4) {
                __m256d values = _mm256_loadu_pd(data + i);
                __m256d mask = _mm256_cmp_pd(values, min_vec, _CMP_LT_OQ);
                
                if (!_mm256_testz_pd(mask, mask)) {
                    // Find minimum in this vector
                    for (t_index j = 0; j < 4; ++j) {
                        if (data[i + j] < min_value) {
                            min_value = data[i + j];
                            min_index = i + j;
                        }
                    }
                    min_vec = _mm256_set1_pd(min_value);
                }
                indices = _mm256_add_epi64(indices, step);
            }
        }
#endif
...
// Use conditional alignment based on platform and C++ standard support
// Strictly check for C++17 aligned allocation support (required for aligned new/delete)
// Only use alignas when we're CERTAIN the runtime supports it

// Priority order:
// 1. Check __cpp_aligned_new feature test macro (most reliable for C++17)
// 2. Check __MAC_OS_X_VERSION_MIN_REQUIRED >= 101300 for macOS (actual OS support)
// 3. ENABLE_ALIGNED_ALLOCATION define (set by setup.py based on deployment target)
#if defined(__cpp_aligned_new) && __cpp_aligned_new >= 201606L
  // C++17 aligned allocation is directly supported - use alignas
  struct alignas(32) node {
    t_index node1, node2;
    t_float dist;
  };
#elif defined(__APPLE__) && defined(__MAC_OS_X_VERSION_MIN_REQUIRED) && __MAC_OS_X_VERSION_MIN_REQUIRED >= 101300
  // macOS 10.13+ has aligned allocation support via libc++
  struct alignas(32) node {
    t_index node1, node2;
    t_float dist;
  };
#else
  // No aligned allocation support
  // - Older macOS (< 10.13): no aligned new/delete in runtime
  // - ENABLE_ALIGNED_ALLOCATION define is just a hint, not a guarantee
  // Performance impact is minimal - SIMD works without alignment
  struct node {
    t_index node1, node2;
    t_float dist;
  };
...
// Optimized indexing functions with cache-friendly access patterns
// D is the upper triangular part of a symmetric (NxN)-matrix
// We require r_ < c_ !
#define D_(r_,c_) ( D[(static_cast<std::ptrdiff_t>(2*N-3-(r_))*(r_)>>1)+(c_)-1] )

// Cache-friendly distance matrix access with prefetching
#define D_CACHE_FRIENDLY(r_,c_) ( \
  __builtin_prefetch(&D[(static_cast<std::ptrdiff_t>(2*N-3-(r_))*(r_)>>1)+(c_)-1], 0, 3), \
  D[(static_cast<std::ptrdiff_t>(2*N-3-(r_))*(r_)>>1)+(c_)-1] \
)

// Z is an ((N-1)x4)-array with cache alignment
...
// Compiler optimization hints (GCC/Clang only)
#ifdef __GNUC__
#pragma GCC optimize("O3,unroll-loops,fast-math")
#endif

inline static void f_single( t_float * __restrict__ const b, const t_float a ) {
  if (__builtin_expect(*b > a, false)) *b = a;
}
inline static void f_complete( t_float * __restrict__ const b, const t_float a ) {
  if (__builtin_expect(*b < a, false)) *b = a;
}
inline static void f_average( t_float * __restrict__ const b, const t_float a, const t_float s, const t_float t) {
  *b = s*a + t*(*b);
  #ifndef FE_INVALID
#if HAVE_DIAGNOSTIC
#pragma GCC diagnostic push
#pragma GCC diagnostic ignored "-Wfloat-equal"
#endif
  if (__builtin_expect(fc_isnan(*b), false)) {
    throw(nan_error());
  }
#if HAVE_DIAGNOSTIC
#pragma GCC diagnostic pop
...
#else
  // Sequential version with SIMD optimization
  t_float const * DD = D;
  for (i=0; i<N_1; ++i) {
    min = std::numeric_limits<t_float>::infinity();
